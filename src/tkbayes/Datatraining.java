...
public class Datatraining {
    ...
    final int jumlah = 15;
     double redtrain[]=new double[jumlah];
    double greentrain[]=new double[jumlah];
    double bluetrain[]=new double[jumlah];
    int diameter[]=new int[jumlah];
    String klasifikasi[]=new String[jumlah];
    int count_train=0;
    double rata2Lemon[] = new double[4], rata2Manis[] = new double[4], rata2Nipis[] = new double[4];
    double invers_covarian[][] = new double[4][4];
    int cl=0,cm=0,cn=0;
    double ratdotinvlemon[] = new double[4], ratdotinvmanis[] = new double[4], ratdotinvnipis[] = new double[4];
    double matriksLemon[][];
    double matriksManis[][];
    double matriksNipis[][];
    ...
    public void covarianGlobal(double[][] a, double[][] b, double[][] c) {
        ...
        matriksManis = b;
        matriksNipis = c;
        double rt2red = 0, rt2green = 0, rt2blue = 0, rtd = 0;
...
//menghitung mean global
        ...
        //System.out.println("sum"+rt2red);
        rt2red = rt2red / jumlah;
        //System.out.println(rt2red);
...
//        for(int i=0;i<4;i++){
//              for (int j = 0; j < 4; j++) {
//                     System.out.print(invers_covarian[i][j]);
//                     }
//                    System.out.println("");
//                      }
    ...
    public void ratdotinv() {
        for (int i = 0; i < ratdotinvlemon.length; i++) {
            for (int j = 0; j < invers_covarian.length; j++) {
                ratdotinvlemon[i] += rata2Lemon[j] * invers_covarian[j][i];
            }
        }
        for (int i = 0; i < ratdotinvmanis.length; i++) {
            for (int j = 0; j < invers_covarian.length; j++) {
                ratdotinvmanis[i] += rata2Manis[j] * invers_covarian[j][i];
            }
        }
        for (int i = 0; i < ratdotinvnipis.length; i++) {
            for (int j = 0; j < invers_covarian.length; j++) {
                ratdotinvnipis[i] += rata2Nipis[j] * invers_covarian[j][i];
            }
        }
    }
